App.controller('createNewSellListing_Step-2Controller', function ($scope, $rootScope, $location, $compile, $http) {

    $scope.values = {};
    $scope.values.productName = "";
    $scope.values.productNotes = "";
    $scope.values.productType = "";
    $scope.values.manufactureDate = "";
    $scope.values.productOriginCity = "";
    $scope.values.productorigincountry = "";
    $scope.values.productOriginState = "";
    $scope.values.packageUnits = "";
    $scope.values.packageType = "";
    $scope.values.packageContainer = "";
    $scope.values.productQuantity = "";
    $scope.values.lotNumber = "";
    $scope.values.pricePerUnit = "";
    $scope.values.totalPrice = "";

    $scope.addProduct = function () {
        var products = $scope.values.productName + $scope.values.productNotes + $scope.values.productType + $scope.values.manufactureDate + $scope.values.productOriginCity + $scope.values.productorigincountry + $scope.values.productOriginState + $scope.values.packageUnits + $scope.values.packageType + $scope.values.packageContainer + $scope.values.productQuantity + $scope.values.lotNumber + $scope.values.pricePerUnit + $scope.values.totalPrice;
        var productContainer = '<div id="" class=" col-sm-12 alert alert-dismissible" style="background-color:#cccccc;font-size:14px" > <button type="button" class="close" data-dismiss="alert"><i class="fa fa-close" style="font-size:36px;"></i></button>' + products + '</div>'
        $("#productHolder").prepend(productContainer);
    };
    ...
    $scope.save = function () {
        localStorage.setItem("product", JSON.stringify($scope.values));
    ...
    $scope.onMyLoad = function () {
        if (localStorage.getItem('product') != "") {
            $scope.values = JSON.parse(localStorage.getItem('product'));
        }
    };
    ...
    $scope.onSaveClose = function () {
        ...
        console.log('product info', localStorage.getItem("product"));
...
// // App.controller('createNewSellListing_Step-2Controller', ['$scope','$rootScope','$compile','$http','$location', function($scope,$rootScope,$compile,$http,$location) {
...
// //     //     var formdata = {
// //     //         "productName":$scope.values.productName,
// // 	// 		"productNotes": $scope.values.productNotes,
// // 	// 		"productType": $scope.values.productType,
// // 	// 		"manufactureDate":$scope.values.manufactureDate,
// // 	// 		"productOriginCity": $scope.values.productOriginCity,
// //     //         "productOriginState": $scope.values.productOriginState,
// //     //         "productOriginCountry": $scope.values.productOriginCountry,
// //     //         "packageUnits": $scope.values.packageUnits,
// //     //         "packageType": $scope.values.packageType,
// //     //         "packageContainer": $scope.values.packageContainer,
// //     //         "productQuantity": $scope.values.productQuantity,
// //     //         "lotNumber": $scope.values.lotNumber,
// //     //         "pricePerUnit": $scope.values.pricePerUnit,
// //     //         "totalPrice": $scope.values.totalPrice,
// //     //         "productId": localStorage.getItem('productId') ? localStorage.getItem('productId') : 0
// //     //     }
...
// //     //     $http({
// //     //         method: 'post',
// //     //         url: 'http://localhost:6060/productdetails/product/create',
// //     //         data: formdata
...
// //     // var listingsdata;
// //     // listingsdata  =JSON.parse( localStorage.getItem("listing"))
// //     //  console.log("First page data",listingsdata);
...
// //             $location.path('/salesOpportunities_Listings');
// //         }
// //         console.log('ASDHAHSDA',localStorage.getItem("product"));
...
// //     if(localStorage.getItem("product")){
// //         $scope.values = JSON.parse(localStorage.getItem("product"));
// //         console.log("asdasdas",$scope.values);
// //     }
